...
vim.o.number = true
-- Relative line numbers (Preserved from your current config)
...
-- Additional settings from your current config
vim.opt.spelllang = { 'en_gb' }
vim.opt.conceallevel = 1

-- [[ Terminal settings state ]] --
local terminals = {
  term = { buf = nil, win = nil, cmd = 'term', split = 'botright 15split', width = nil, height = 15 },
  opencode = { buf = nil, win = nil, cmd = 'term opencode', split = 'botright vsplit', width = 60, height = nil },
}
  ...
  for _, t in pairs(terminals) do
    if t.win and vim.api.nvim_win_is_valid(t.win) then
      if t.height then
        ...
        if current_h ~= t.height then
          vim.api.nvim_win_set_height(t.win, t.height)
        end
      end
      if t.width then
        local current_w = vim.api.nvim_win_get_width(t.win)
        if current_w ~= t.width then
          vim.api.nvim_win_set_width(t.win, t.width)
        end
      end
    end
  end
...
require('lazy').setup({
  ...
  { -- Fuzzy Finder (files, lsp, etc)
    ...
    config = function()
      ...
      -- Advanced Search (Hidden/Ignored Files) from your config
      ...
      -- Advanced Search (Config with hidden files)
      vim.keymap.set(
        'n',
        '<leader>sn',
      ...
      -- LSP Telescope Keymaps (LspAttach)
...
vim.keymap.set('n', '<leader>o', function() toggle_terminal('opencode') end, { desc = 'Toggle [O]pencode' })
